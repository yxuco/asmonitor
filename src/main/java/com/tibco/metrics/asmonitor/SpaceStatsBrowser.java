...
public class SpaceStatsBrowser {
	...
	public SpaceStatsBrowser(String metaspaceName, String discovery, String memberName) throws Exception {
	...
	private void connectAS() throws Exception {
		MemberDef memberDef = MemberDef.create(memberName,
				discovery, null);
	...
	public void closeWriter() {
		if (writer != null) {
			try {
				System.out.println(String.format("Close writer for metaspace %s on discovery %s", metaspaceName, discovery));
				writer.close();
	...
	/**
	 * Collect AS stats, and write them to stat log file.  can be called by separate worker threads.
	...
	public void collectMetrics()
	{
		...
		try {
			...
			System.out.println(String.format("Failed to write stats of metaspace %s: %s", metaspaceName, e.getMessage()));
			closeWriter();
			try {
				writeMetrics(timestamp);
			} catch (IOException io) {
				System.out.println(String.format("Exception while writing stat of %s: %s", metaspaceName, io.getMessage()));
				closeWriter();
			}
		}
		finally {
	...
	public void writeMetrics(String timestamp) throws IOException
	{
		...
		try {
			...
			writer.write(String.format("Failed to get stats for metaspace %s: %s\n", metaspaceName, ex.getMessage()));
	 ...
	 * 
	 * @throws IOException when the AS stat file no longer exist or not writable.
	 */
